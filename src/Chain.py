...
class Chain:
    ...
    def draw(self, screen, color=(0, 0, 0)):
        for i in range(len(self.joints) - 1):
            pygame.draw.line(screen, color, self.joints[i], self.joints[i + 1], 3)
            pygame.draw.circle(screen, color, (int(self.joints[i][0]), int(self.joints[i][1])), 5)
        pygame.draw.circle(screen, color, (int(self.joints[-1][0]), int(self.joints[-1][1])), 5)